Here's the formatted README content that you can directly copy and paste:
...
3. **Configure the databases**
   ...
   The application is configured to use a Neon PostgreSQL database. For local development, you may want to update the database configuration in each service's `application.yml` file.
...
The application uses JWT (JSON Web Tokens) for authentication. The token needs to be included in the `Authorization` header for protected endpoints:
...
- Initial focus on clean architecture principles and their application to microservices
- Database design for user/auth service
- Implementation of hexagonal architecture with domain-driven design
- Development of key APIs for user management, project sharing, and skill management
- Integration of Kafka for event-driven communication
- Implementation of JWT token authentication
- Setup of service discovery using Netflix Eureka
- Configuration of API Gateway for request routing
...
This markdown content includes all the sections from your document with proper formatting. The image links use Imgur URLs for the diagrams we created earlier, so they will display properly in your README. You can simply copy and paste this entire content into your README.md file.
